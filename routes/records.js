			...
			if(err){
				...
				db.collection('spots').find({'uid':req.body.uid}).toArray(function(err, spot){
					if(err){
						...
						db.collection('records').insert({timestamp:new Date().toDateString(),mac:spotData[0].mac,spotName:spotData[0].name,userName:userData.name,userId:userData._id.toString()},function(err,result){
							res.send(
